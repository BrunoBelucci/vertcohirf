...
class Splitter(abc.ABC):
    ...
    def split(self, *Xs, indices=None, allow_empty_party=False, fill=None, **kwargs):
        ...
        if n_features < self.num_parties:
            raise ValueError(f"Number of features ({n_features}) must be >= number of parties ({self.num_parties})")
        ans = []
...
class ImportanceSplitter(Splitter):
    def __init__(self, num_parties, weights=1.0, seed=None):
        """
    ...
    def split_indices(self, X, allow_empty_party=False, **kwargs):
        ...
        if allow_empty_party:
            ...
            # no empty party
            assert (
                np.count_nonzero([len(party) for party in party_to_feature]) == self.num_parties
            ), "There should be no empty party"
            assert (
                np.sum([len(party) for party in party_to_feature]) == X.shape[1]
...
class CorrelationSplitter(Splitter):
    ...
    def __init__(self, num_parties: int, evaluator: CorrelationEvaluator = None, seed=None, gpu_id=None, n_jobs=None):
        ...
        if self.gpu_id is not None:
            assert (
                self.evaluator.gpu_id == self.gpu_id
    ...
    def check_fit_data(self):
        ...
        assert (
            self.evaluator.n_features_on_party is not None
        ), "self.evaluator.n_features_on_party is None. Please call fit() first."
    ...
    def split_indices(
        ...
        **kwargs,
        ...
        res_beta = minimize(
            self.CorrBestMatchProblem(
                self.evaluator.corr,
                self.evaluator.n_features_on_party,
                beta,
                self.min_icor,
                self.max_icor,
