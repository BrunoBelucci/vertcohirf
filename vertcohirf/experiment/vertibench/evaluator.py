...
class ImportanceEvaluator:
    ...
    def check_data(Xs):
        ...
        assert all(
            [X.shape[0] == Xs[0].shape[0] for X in Xs]
    ...
    def evaluate_feature(self, X, model: callable, **kwargs):
        ...
        if sample_size <= 1:
            raise ValueError(
                f"Sample size 0 cannot produce meaningful Shapley values." f" Please increase the sample rate."
            )
...
class CorrelationEvaluator:
    ...
    def pcor_singular_approx(corr, n_components=400, n_oversamples=10, n_iter=4, random_state=0):
        ...
        _, singular_values, _ = randomized_svd(
            corr, n_components=n_components, n_oversamples=n_oversamples, n_iter=n_iter, random_state=random_state
        )
    ...
    def pcor_singular_approx_gpu(corr: torch.Tensor, n_components=400, n_iter=4):
        ...
        singular_shape = min(corr.shape)
        s_append_zero = torch.concatenate(
            (singular_values, torch.zeros(singular_shape - singular_values.shape[0]).to(corr.device))
        )
    ...
    def fit_min_max(
        self, n_elites=200, n_offsprings=700, n_mutants=100, n_gen=100, bias=0.7, verbose=False, seed=None, **kwargs
    ):
        """
        ...
        if self.corr is None:
            raise ValueError(
                "Data matrix X is not provided, and self.corr is not calculated yet."
        ...
        res_min = minimize(
            self.CorrMinProblem(self.corr, self.n_features_on_party, evaluator=self, runner=self.runner),
            algorithm,
            ("n_gen", n_gen),
            seed=seed,
            verbose=verbose,
        ...
        res_max = minimize(
            self.CorrMaxProblem(self.corr, self.n_features_on_party, evaluator=self, runner=self.runner),
            algorithm,
            ("n_gen", n_gen),
            seed=seed,
            verbose=verbose,
