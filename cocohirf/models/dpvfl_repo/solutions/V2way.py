...
class V2way(VPrivClustering):
    ...
    def fit(self, data, run_clean: bool = True, true_labels: np.array = None):
        ...
        grids, intersection_counts, clean_intersection_counts, sample_to_grid_indices = self.build_weighted_grids(
            n, data, centers, clean_memberships, local_aggregation_eps, local_k, run_clean
        )
    ...
    def build_weighted_grids(self, n, data, centers, clean_memberships, eps, local_k, run_clean):
        ...
        if run_clean:
            ...
            return grids, grid_weights_df.values.flatten(), clean_intersection_counts, sample_to_grid_indices
        else:
            return grids, grid_weights_df.values.flatten(), [], sample_to_grid_indices
